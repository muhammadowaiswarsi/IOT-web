        ...
        return (
            <div className="Main">
                ...
                <div className="input-main-div flex-center-center">
                    <label className="input-label flex-between-center"> Input Here:<input value={this.state.input} onChange={this.changeValue} /></label>
                    <Button className="func-button" onClick={this.submitFunc}>{addLoader ? <ReactLoading type="spin" width="20px" color="grey" /> : "submit"}</Button>
                </div>
                    ...
                    {data && data.length && data.map((item, index) => {
                        return (
                            <Col key={index} className="Item_main_Col MB20" md={3} lg={3} sm={11} xs={11}>
                                ...
                                <Button className="func-button" onClick={() => this.removeFunc(item, index)}>remove</Button>
                            </Col>
                        )
                    })}
                </div>
